...
//
	...
	v = DotProduct (ent->v.velocity, ent->v.velocity);

	if ((v > sv_maxvelocity.value*sv_maxvelocity.value) && (v))
	{
		v = sv_maxvelocity.value / sqrt (v);
		VectorScale (ent->v.velocity, v, ent->v.velocity);
	}
}
