...
char *PR_ValueString (etype_t type, eval_t *val)
{
	...
	{
	case ev_string:
		_snprintf (line, sizeof(line), "%s", pr_strings + val->string);
		break;
	case ev_entity:	
		_snprintf (line, sizeof(line), "entity %i", NUM_FOR_EDICT(PROG_TO_EDICT(val->edict)) );
		break;
	case ev_function:
		f = pr_functions + val->function;
		_snprintf (line, sizeof(line), "%s()", pr_strings + f->s_name);
		break;
	case ev_field:
		def = ED_FieldAtOfs ( val->_int );
		_snprintf (line, sizeof(line), ".%s", pr_strings + def->s_name);
		break;
	case ev_void:
		_snprintf (line, sizeof(line), "void");
		break;
	case ev_float:
		_snprintf (line, sizeof(line), "%5.1f", val->_float);
		break;
	case ev_vector:
		_snprintf (line, sizeof(line), "'%5.1f %5.1f %5.1f'", val->vector[0], val->vector[1], val->vector[2]);
		break;
	case ev_pointer:
		_snprintf (line, sizeof(line), "pointer");
		break;
	default:
		_snprintf (line, sizeof(line), "bad type %i", type);
		break;
	}
...
char *PR_UglyValueString (etype_t type, eval_t *val)
{
	...
	case ev_string:
		...
		break;
	case ev_entity:	
		_snprintf (line, sizeof(line), "%i", NUM_FOR_EDICT(PROG_TO_EDICT(val->edict)));
		break;
	case ev_function:
		f = pr_functions + val->function;
		_snprintf (line, sizeof(line), "%s", pr_strings + f->s_name);
	...
	case ev_void:
		...
		break;
	case ev_float:
		_snprintf (line, sizeof(line), "%f", val->_float);
		break;
	case ev_vector:
		_snprintf (line, sizeof(line), "%f %f %f", val->vector[0], val->vector[1], val->vector[2]);
		break;
	default:
		_snprintf (line, sizeof(line), "bad type %i", type);
		break;
...
char *PR_GlobalString (int ofs)
{
	...
	if (!def)
		_snprintf (line,sizeof(line),"%i(???)", ofs);
	else
	{
		s = PR_ValueString (def->type, val);
		_snprintf (line,sizeof(line),"%i(%s)%s", ofs, pr_strings + def->s_name, s);
	}
...
char *PR_GlobalStringNoContents (int ofs)
{
	...
	if (!def)
		...
		_snprintf (line,sizeof(line),"%i(%s)", ofs, pr_strings + def->s_name);
...
_snprintf (com_token, sizeof(com_token),"0 %s 0", temp);
}
